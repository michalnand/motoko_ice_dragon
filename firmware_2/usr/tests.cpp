...
void shaper_test()
{
  ...
  //input shaper init
  shaper_forward.init(0.1, 0.1, 0.01, 0.01);   
  ...
  //float u_forward_req_list[] = {0.5, 0.0, -0.5, 0.0};
  //float u_turn_req_list[]    = {0.0, 0.0,  0.0, 0.0};

  float u_forward_req_list[] = {0.0, 0.0,  0.0, 0.0};
  float u_turn_req_list[]    = {0.5, 0.0,  -0.5, 0.0};

  uint32_t ptr = 0; 

  while(1)
  {
    float u_forward_req   = u_forward_req_list[ptr];
    float u_turn_req      = u_turn_req_list[ptr];

    ptr = (ptr + 1)%4;

    for (unsigned int n = 0; n < n_steps; n++)
    {
      float u_forward_sh = shaper_forward.step(u_forward_req);
      float u_turn_sh    = shaper_turn.step(u_turn_req);

      float u_left  = u_forward_sh - u_turn_sh;
      float u_right = u_forward_sh + u_turn_sh; 

      u_left  = speed_max*clip(u_left, -1.0, 1.0); 
      u_right = speed_max*clip(u_right, -1.0, 1.0);
